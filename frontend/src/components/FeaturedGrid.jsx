...
export default function FeaturedGrid({ lang = 'zh', userPermissions = ['Fab课程', 'AI应用基础'] }) {
  const t = {
    ...
    en: {
      ...
      cards: [
        ...
        {
          title: 'CMF Applications',
          description: 'Study industrial aesthetics and physical tactile finishes. Define standards for colors, anodizing thickness, and sandblasting texture grains.',
          tag: '',
        }
      ]
    }
  ...
  const cardsData = [
    ...
    },
    {
      ...
      tag: t.cards[3].tag,
      icon: <CmfThinkingIcon />,
      link: '#/courses/cmf',
      permissionKey: 'CMF应用',
      svg: <CmfApplicationsVisual />
    }
  ];
  ...
  return (
    <section id="showcase" className="container-custom" style={{ paddingBlock: '6rem 4rem' }}>
      ...
      <div className="grid-featured" style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: '2rem' }}>
        {cardsData.map((card, idx) => {
          const isPermitted = hasPermission(card.permissionKey);
          return (
            <div 
              key={idx} 
              className="grid-col-3 glassmorphism-card sophigo-card"
              style={{
                gridColumn: 'span 3',
            ...
            >
              {/* SVG Media container */}
              <div className="sophigo-card-media" style={{ height: '140px', position: 'relative' }}>
                {card.svg}
              </div>
